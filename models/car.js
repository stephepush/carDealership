...
module.exports = class Car {
    ...
    static fetchAll() {
        const options = {
            sql: "SELECT c.car_id, c.model_year, c.make, c.miles, c.color, c.transmission, c.layout, c.engine_type, p.car_id, p.car_photo_url FROM cars c INNER JOIN car_photos p ON c.car_id = p.car_id",
            nestTables: true
        };
    ...
    save() {
        ...
        .then((db) => {
            ...
            .then(() => {
                const car_insert = db.query('INSERT INTO cars (model_year, make, model, color, miles, transmission, layout, engine_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [this.model_year, this.make, this.model, this.color, this.miles, this.transmission, this.layout, this.engine_type]);
                return car_insert;
            })
            .then((car_insert) => {
            ...
            .then(() => {
                //console.log(pk)
                db.query('INSERT INTO car_photos (car_id, car_photo_url) VALUES (?,?)', [pk, this.car_photo_url])
            })
            .then(() => {
                db.query('INSERT INTO car_prices (car_id, car_price) VALUES (?, ?)', [pk, this.car_price])
            })
            .then(() => {
                db.query('INSERT INTO sales_status (car_id, sale_status, for_sale) VALUES (?, ?, ?)', [pk, this.sale_status, this.for_sale])
            })
            .then(() => {
